...
tree_t * search_parent(tree_t*t,int v,tree_t *parent)
{
    tree_t *pos;
    if  (t == NULL)
        return t;
...
void print_path(tree_t *t, int start, int end)
{
    ...
    {
     stk_t *path = NULL;
     tree_t *str=search_pos(t,start),*pos=search_pos(t,end);
     path = push(path,end);
     int addparent = search_pos(t,end)->data,parent;
     while(pos != str&&pos != NULL)
     {
      if(search_parent(t,addparent,NULL)!= NULL)
        parent = search_parent(t,addparent,NULL)->data;
      else 
        parent = NULL;
      if (parent != NULL)
        path = push(path,parent);
      addparent = search_pos(t,parent)->data;
      pos = search_pos(t,addparent);
     }
     while(path!=NULL)
     {
        printf("%d ",path->data);
...
{
    tree_t *start = search_pos(t,v),*tmp = start->next_sibling;
...
void test(tree_t*t ,tree_t *root)
{
  if(t == NULL)
      return;
  for(int i=0;i<4*depth(root,t->data);i++)
      printf(" ");
  printf("%d\n",t->data);
  test(t->first_child,root);
  test(t->next_sibling,root);
}
