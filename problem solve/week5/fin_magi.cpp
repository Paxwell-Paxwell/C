...
vector<int> magic;
int magic1= 0;
set<int> bfs_magic(int hp,int start){
    list<int> Q;
    int levels[n];
    bool seen[n];
    set<int> visited;
        ...
        for(int d=0; d<adj[u].size(); d++) {
                ...
                if(!seen[v]) {
                    ...
                    if(!magic1&&find(magic.begin(), magic.end(), v+1) != magic.end()){
                        //cout<<"magic--> "<<v<<"\n";
                        set<int> temp = bfs_magic(h-levels[v],v);
                        set_union(vi_with_magic.begin(), vi_with_magic.end(), temp.begin(), 
                        temp.end(),inserter(vi_with_magic, vi_with_magic.begin()));
                    }
